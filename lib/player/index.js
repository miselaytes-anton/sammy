...
function Player() {
    ...
    rs._read = function (bytes) {
        ...
        //alternative read
        var buf = new Buffer(4096);// mac->fine, raspberry -> fine
        var n;
        for (var i = 0; i < buf.length-1; i++) {
            if(typeof self.sound[self.pointer] ==='undefined' || self.sound[self.pointer] ===null){
                self.pointer = 0;
            }
            n = self.sound[self.pointer];
