...
function Player() {
    ...
    rs._read = function (bytes) {
       // console.log(bytes);
        ...
        for (var i = 0; i < buf.length-1; i++) {
            //console.log(self.sound.length, self.soundCopy.length);
            if(!self.sound.length){
                self.sound= _.clone(self.soundCopy);
                //console.log('renewing', self.soundCopy.length);
                //rs.push(null);
            }
            n = self.sound.shift();
    ...
    self.play = function (sound) {
        //console.log(JSON.stringify(sound), sound.length);
        self.sound = sound;
        self.soundCopy = _.clone(sound);
        if (!self.isPlaying) {
            rs.pipe(speaker);
            self.isPlaying = true;
        }
